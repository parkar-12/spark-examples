...
public class myKafkaAppender extends KafkaLog4jAppender {
    private String brokerList;
    private String topic;
    private String filter_log_level;
    ...
    public String getfilter_log_level() {
        return filter_log_level;
    }
    public void setfilter_log_level(String filter_log_level) {
        this.filter_log_level = filter_log_level;
    }
...
// check for config parameter validity
    ...
    public List convertToArray(String stringToMatch){
        List<String> stringToMatchList = new ArrayList<String>(Arrays.asList(stringToMatch.split(",")));
        return stringToMatchList;
    }

    protected String filter(String message, String filter_log_level, List<String> stringToMatch, boolean acceptOnMatch){

        System.out.println("filter_log_level:- " +filter_log_level);
        System.out.println("stringToMatch:- " +stringToMatch);
        System.out.println("acceptOnMatch:- " +acceptOnMatch);
        if(filter_log_level==null && stringToMatch==null){
            return message;
        }
        if(acceptOnMatch){
            if(filter_log_level==null||message.contains(filter_log_level)){
                for(String store: stringToMatch) {
                ...
                if(message.contains(filter_log_level)){
                    return message;
                }
            }
            return null;
    ...
    protected void append(LoggingEvent event) {
        String logging_message = subAppend(event);
        LogLog.debug("[" + new Date(event.getTimeStamp()) + "]" + logging_message);
        List stringToMatchList=convertToArray(stringToMatch);
        String message=filter(logging_message,filter_log_level,stringToMatchList,acceptOnMatch);
        if(message!=null){
    ...
    private String subAppend(LoggingEvent event) {
      //  System.out.println(event.getRenderedMessage()+" "+event.getPropertyKeySet());
